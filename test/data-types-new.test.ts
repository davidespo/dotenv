...
describe("Dotenv Data Type Parsing - New Contract", () => {
  beforeEach(() => {
    // Clear process.env
...
TEST_DISABLED=disabled
  ...
  describe("Multi-line Strings - New Contract", () => {
    it("should parse single-line quoted strings", async () => {
      const envContent = `
TESTING="this is only a test"
MESSAGE=testing use case
      `;
      ...
      expect(config.TESTING).toBe("this is only a test");
      expect(config.MESSAGE).toBe("testing use case");
    });

    it("should parse multi-line strings with double quotes", async () => {
      const envContent = `
MESSAGE="# title
Just testing some multi-line
markdown"
      `;
      ...
      expect(config.MESSAGE).toContain("# title");
      expect(config.MESSAGE).toContain("Just testing some multi-line");
      expect(config.MESSAGE).toContain("markdown");
    });

    it("should handle multi-line strings with escaped quotes", async () => {
      const envContent = `
MESSAGE="# title
Testing a markdown quote
> \"Hello, World!\"
> - anonymous"
      `;
      ...
      expect(config.MESSAGE).toContain("# title");
      expect(config.MESSAGE).toContain("Testing a markdown quote");
      expect(config.MESSAGE).toContain('> "Hello, World!"');
      expect(config.MESSAGE).toContain("> - anonymous");
    });

    it("should handle single-line quoted strings with escaped quotes", async () => {
      const envContent = `
TEST_ESCAPED_SINGLE="This has \"quotes\" inside"
      `;
      ...
      const config = await Dotenv.load({
        filepaths: [".env"],
        schema: z.object({
          TEST_ESCAPED_SINGLE: z.string(),
        }),
      });

      expect(config.TEST_ESCAPED_SINGLE).toBe('This has "quotes" inside');
    });
...
TEST_MULTIPLE_EQUALS=a=b=c=d
    ...
    it("should handle values with quotes in the middle (unquoted)", async () => {
      const envContent = `
...
"
    ...
    it("should handle single quotes as regular characters", async () => {
      const envContent = `
TEST_SINGLE_QUOTES=This has 'single' quotes
TEST_SINGLE_QUOTED="This has 'single' quotes"
      `;
      ...
      const config = await Dotenv.load({
        filepaths: [".env"],
        schema: z.object({
          TEST_SINGLE_QUOTES: z.string(),
          TEST_SINGLE_QUOTED: z.string(),
        }),
      });

      expect(config.TEST_SINGLE_QUOTES).toBe("This has 'single' quotes");
      expect(config.TEST_SINGLE_QUOTED).toBe("This has 'single' quotes");
    });
  });
});
